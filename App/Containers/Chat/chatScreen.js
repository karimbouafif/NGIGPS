  ...
  getMessages = async () => {
    try {
      let response = await axios.get(
        'http://192.168.1.16:4000/api' +
        '/chats/chats/60b0c7026699d7727c4b82f7'
      );
      if (response.status === 200) {
        ...
        for (var i = 0; i < response.data.length; i++) {
          if (response.data[i].sender == this.state.userId) {
            await axios
              .get(
                'http://192.168.1.16:4000/api/users/find/' +
      ...
      <View>
        ...
        <View>
          <ScrollView
            ...
            <TouchableOpacity
              onPress={() => {
                this.props.navigation.navigate('Root', { screen: 'Broadcast' }, {
            ...
            {this.state.chats.map(chatItem => (
              <TouchableOpacity
                onPress={() => {
                  this.props.navigation.navigate('Root', { screen: 'Messages' }, {
